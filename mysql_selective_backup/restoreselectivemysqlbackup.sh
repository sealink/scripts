...
# MySQL owner
# Replace with ${USER} for sandbox
if ! $SANDBOX ; then
MYSQL_SYS_USER=mysql
MYSQL_SYS_GROUP=mysql
fi
 
TARGET_DATABASE_PATH=${TARGET_DIR}/$TARGET_DATABASE
...
cp -f $TABLE.exp "$TARGET_DATABASE_PATH/$TABLE.exp"
chown $MYSQL_SYS_USER:$MYSQL_SYS_GROUP "$TARGET_DATABASE_PATH/$TABLE.exp"
cp -f $TABLE.ibd "$TARGET_DATABASE_PATH/$TABLE.ibd"
chown $MYSQL_SYS_USER:$MYSQL_SYS_GROUP "$TARGET_DATABASE_PATH/$TABLE.ibd"
